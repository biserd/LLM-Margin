...
- **Backend** (`artifacts/api-server`):
  ...
  - `routes/stripe.ts`:
    - `POST /api/stripe/checkout` — **no auth required**. Body `{ lookupKey }`. If the caller is signed in, reuses/creates their Stripe customer, sets `client_reference_id` + `subscription_data.metadata.userId`, and dedupes against existing active subs (returns the billing-portal URL instead of charging twice). If anonymous, no customer is pre-created and Stripe Checkout collects the email itself. Returns `{ url }`.
