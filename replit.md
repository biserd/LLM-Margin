...
- **Backend** (`artifacts/api-server`):
  ...
  - `routes/stripe.ts`:
    - `POST /api/stripe/checkout` — **no auth required**. Body `{ lookupKey }`. Creates a subscription Checkout Session with a **7-day free trial** (`subscription_data.trial_period_days: 7`) and `payment_method_collection: "always"` so the card is captured up front and the sub auto-converts at trial end. If the caller is signed in, reuses/creates their Stripe customer, sets `client_reference_id` + `subscription_data.metadata.userId`, and dedupes against existing active/trialing subs (returns the billing-portal URL instead of starting another trial). If anonymous, no customer is pre-created and Stripe Checkout collects the email itself. Returns `{ url }`.
    - `GET /api/stripe/checkout/complete?session_id=cs_…` — Stripe success URL. Retrieves the session, verifies it's paid, then branches on whether the email Stripe collected matches an existing user: **(a) brand-new email** → create user, link `stripe_customer_id`, apply subscription state immediately (no webhook race), issue an `lm_session` cookie, redirect to `/account?upgraded=1` (auto-sign-in is safe here because there was no prior account to take over); **(b) existing email** → apply the subscription to that user (they paid for it; only links `stripe_customer_id` if they had none — never overwrites), but do NOT issue a session; redirect to `/sign-in?next=/account?upgraded=1&email=…&upgraded=1` so the buyer must OTP-verify ownership of the email before getting a cookie. This prevents account takeover via paid-checkout-with-victim's-email. In both cases backfills `subscription.metadata.userId` so future webhook events resolve correctly.
    ...
    - `POST /api/stripe/webhook` — exported as `webhookRouter`, **mounted in `app.ts` BEFORE `express.json()`** with `express.raw({ type: 'application/json' })` so signature verification works. Handles `checkout.session.completed`, `customer.subscription.created/updated/deleted`. `applySubscription()` resolves the user via `subscription.metadata.userId` (preferred) or `stripe_customer_id` lookup. The `checkout.session.completed` handler also has an anonymous-checkout fallback: if there's no `client_reference_id`/`metadata.userId` on the session, it upserts a user from the Stripe-collected email so the entitlement is never lost even if the buyer never returns to `/complete` (no session cookie is ever issued from a webhook — the buyer must still OTP-sign-in to claim the account). Webhook arrival before `/complete` is fine — `applySubscription` is idempotent.
...
- **Frontend** (`artifacts/tokencalc`):
  - `src/lib/billing.ts` — `startCheckout(lookupKey)` (POST `/api/stripe/checkout`, redirects to returned URL — works for both signed-in and anonymous users; no auth gate) and `openBillingPortal()` (still auth-gated).
  - `src/lib/auth.tsx` — `AuthUser` extended with plan fields.
  - `src/pages/Pricing.tsx` — Both CTAs say "Start 7-day free trial" and call `startCheckout` with the price matching the monthly/annual toggle. If the user is already Pro, the buttons become "Manage subscription" linking to `/account`. Reads `?canceled=1` for the cancel banner. Renders the shared `LogoStrip` and `Testimonials` social-proof sections from `components/SocialProof.tsx`.
  - `src/components/SocialProof.tsx` — exports `LogoStrip`, `StatsStrip`, `Testimonials`. Used on the homepage (MarginSimulator) and on Pricing. Names/companies/quotes are illustrative (not real customers) — explicitly requested by the product owner. If you ever onboard real reference customers, swap them in here.
  - `src/pages/SignInPage.tsx` — After successful sign-in honors `?next=`. Also reads `?email=` (prefills the email field) and `?upgraded=1` (shows a "Thanks for your purchase!" banner). These are used when an anonymous buyer pays with an email that matches an existing account — `/api/stripe/checkout/complete` redirects them here so they OTP-verify before getting a session.
  - `src/pages/AccountPage.tsx` — Shows current plan + "Manage billing" (portal) or "Upgrade" link. When `subscriptionStatus === "trialing"` shows "Pro — Free trial" with a "Free until {date} — first charge that day" line so users know exactly when they'll be billed. After `?upgraded=1` it polls `/auth/me` every 1.5s up to 6 times to wait for the webhook to flip the plan.
