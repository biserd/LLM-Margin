...
- **Email**: `lib/email.ts` uses Resend if `RESEND_API_KEY` is set, otherwise logs the OTP code to the api-server console (dev fallback). If Resend rejects the message (e.g. unverified `from` domain — by default we use `onboarding@resend.dev` which can only deliver to the Resend account owner's email), we log the error AND log the OTP code, and still return 200 to the client so sign-in is never blocked by a Resend setup issue. `EMAIL_FROM` env var optional, defaults to `LLM Margin <onboarding@resend.dev>` — set to a verified-domain address (e.g. `auth@yourdomain.com`) once you verify a domain at https://resend.com/domains to deliver to arbitrary recipients.
